...
import {
  beginAntigravityBrowserLogin,
  beginAntigravityHeadlessLogin,
  refreshAntigravityToken,
} from './oauth.js';

let refreshPromise: ReturnType<typeof refreshAntigravityToken> | undefined;

const AntigravityAuthPlugin: Plugin = async ({ client }) => ({
  auth: {
    ...
    async loader(getAuth) {
      ...
      if (auth.expires > Date.now()) return { apiKey: auth.access, projectId: accountId };
    ...
    methods: [
      {
        ...
        async authorize() {
          ...
          return {
            url: login.url,
            instructions:
              'Complete Google sign-in in your browser. OpenCode captures the callback.',
            method: 'auto',
            async callback() {
              try {
      ...
      },
      {
        ...
        async authorize() {
          ...
          return {
            url: login.url,
            instructions:
              'Complete Google sign-in, then paste the full localhost callback URL from the browser.',
            method: 'code',
            async callback(code) {
              try {
