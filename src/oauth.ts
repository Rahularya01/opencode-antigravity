...
function apiHeaders(access: string): Record<string, string> {
  return {
...
async function discoverProject(access: string): Promise<string | undefined> {
...
export function parseAntigravityCallback(raw: string, state: string): string {
  const text = raw.trim();
  let url: URL;
  try {
    url = new URL(text);
  ...
  if (error) throw new Error(`Google OAuth failed: ${error.slice(0, 200)}`);
  if (url.searchParams.get('state') !== state) throw new Error('Google OAuth state mismatch');
  const code = url.searchParams.get('code');
  if (!code) throw new Error('Google OAuth callback is missing its code');
  return code;
}

async function exchangeCode(code: string, verifier: string): Promise<AntigravityOAuthCredentials> {
...
function waitForCallback(state: string): Promise<{ code: Promise<string>; close(): void }> {
  return new Promise((resolve, reject) => {
    let settle!: (code: string) => void;
    let fail!: (error: Error) => void;
    const code = new Promise<string>((done, failed) => {
      settle = done;
      fail = failed;
    });
    const server = createServer((request, response) => {
      try {
        if (request.method !== 'GET' || !request.url) throw new Error('Invalid OAuth callback');
...
export async function beginAntigravityBrowserLogin(): Promise<{
  url: string;
...
export function beginAntigravityHeadlessLogin(): {
  ...
  return {
    url: login.url,
    complete: (callback) =>
      exchangeCode(parseAntigravityCallback(callback, login.state), login.verifier),
  };
}
